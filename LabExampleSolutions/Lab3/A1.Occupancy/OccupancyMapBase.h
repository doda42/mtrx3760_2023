...
// 2) test if a given location has been observed as being occupied
//
...
class COccupancyMapBase
{
  public:
    ...
    // Loads a local file called ObservationsFName containing observations of occupied map locations.
  ...
  private:
    //--Data------------------------------------------------------------------------------------------------
    std::vector<std::pair<int,int>> mObservedPoints;    // A set of test points that correspond to obstacles
    std::vector<std::pair<int,int>> mNotObservedPoints; // A set of test points that correspond to free space
   
    //--Helper Functions------------------------------------------------------------------------------------
    
    // load a file of test points for testing
    void PopulateForEval( std::string ObservationsFName, std::string NotObservedFName );   
    
    // validate and time a derived-class map, returns runtime in ms
    double FindTotRuntime();                     
};
